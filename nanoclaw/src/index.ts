...
/**
 * Auto-register a DM chat if not already registered.
 * DMs don't require trigger and respond directly.
 */
function autoRegisterDM(jid: string, senderName?: string): void {
  if (registeredGroups[jid]) return; // Already registered

  const phoneNumber = jid.split('@')[0];
  const folderName = `dm-${phoneNumber}`;
  const displayName = senderName || phoneNumber;
  ...
  logger.info({ jid, name: displayName }, 'DM auto-registered');
}
...
function isDockerMode(): boolean {
  // Use Docker on Windows and Linux, Apple Container on macOS
...
function ensureContainerSystemRunning(): void {
  if (isDockerMode()) {
    ...
    try {
      ...
      console.error(
        '║  Agents require Docker to run. To fix:                        ║',
      );
      console.error(
        '║  1. Install Docker Desktop from: https://docker.com/download  ║',
      );
      console.error(
    ...
    // Kill and clean up orphaned NanoClaw containers from previous runs (Docker)
    try {
