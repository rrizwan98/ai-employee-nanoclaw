...
export class WhatsAppChannel implements Channel {
  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock = makeWASocket({
      ...
      // printQRInTerminal is deprecated, we handle QR display manually
      logger,
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (qr) {
        ...
        console.log('║  3. Scan the QR code shown below                              ║');
        console.log('╚════════════════════════════════════════════════════════════════╝\n');
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        // Always notify about chat metadata for group/DM discovery
        const isGroup = chatJid.endsWith('@g.us');
        const isDM = chatJid.endsWith('@s.whatsapp.net');
        this.opts.onChatMetadata(chatJid, timestamp, undefined, 'whatsapp', isGroup);

        // Deliver full message for registered groups OR DMs
        const groups = this.opts.registeredGroups();
        // DMs are always processed (auto-registered in index.ts onMessage callback)
